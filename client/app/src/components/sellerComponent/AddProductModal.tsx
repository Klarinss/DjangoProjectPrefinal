...
export default function AddProductModal({ modalActive, onChange }: ModalProps) {
 const [imagePreview, setImagePreview] = useState('')

    const [formData, setFormData] = useState<AddProductsProps>({
        id: 0,
    ...
    useEffect(() => {
     ...
     const handleSubmit= async (e:React.FormEvent)=>{
        ...
        try{
            const response = await axios.post<AddProductsProps>('http://127.0.0.1:8000/kupal/products/',formData,
                {
                headers: {
